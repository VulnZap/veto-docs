...
### 1. Zero-config local mode

Use local `./veto` rules automatically (or allow-all fallback when none is found):
...
### 4. Advanced control with `Veto.init()`
...
- `mode: 'shadow'` is currently treated as `mode: 'log'`.
