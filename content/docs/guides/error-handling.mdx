...
For standalone checks via `veto.guard(...)`, log mode is intentionally different: it returns the real verdict (`deny` / `require_approval`) so callers can decide what to do.
