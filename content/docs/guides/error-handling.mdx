...
## Shadow mode: test policies without blocking
...
console.log(stats.deniedCalls); // increments in shadow mode
```
</Tab>
<Tab value="Python">
```python
# mode="shadow" in your Veto config
veto = await Veto.init()
wrapped = veto.wrap(tools)
...
# Entries include shadow markers in metadata/context when applicable
```
</Tab>
</Tabs>

In contrast, `log` mode is a compatibility mode that overrides wrapped-call denies to `allow`.

See [Modes](/docs/concepts/modes) for strict vs log vs shadow behavior.
...
5. **Use shadow mode first** — test your policies against real traffic before switching to strict mode
