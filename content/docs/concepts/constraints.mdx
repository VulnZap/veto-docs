...
description: Every constraint type supported by Veto — policy constraints, YAML rule operators, and presence checks.
---
...
Policy constraints are configured per-tool in the [Veto dashboard](https://runveto.com) or via the policy API. They apply to both client-side and server-side deterministic validation.
