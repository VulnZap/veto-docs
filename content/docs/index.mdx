...
- **CLI tooling** — `veto`/`veto repl` (interactive policy REPL), `veto compile` (NL → rules), `veto learn` (observe → generate), `veto test` (gap analysis), `veto scan` (coverage audit), `veto diff` (policy impact diff)
