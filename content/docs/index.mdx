...
```yaml title="veto/rules/defaults.yaml"
rules:
  - id: limit-transfers
    action: block
...
- **CLI tooling** — `veto`/`veto studio` (Veto Studio interactive mode), `veto policy generate|apply`, `veto guard check` (headless), `veto cloud login` (device auth), plus `veto compile`, `veto learn`, `veto test`, `veto scan`, and `veto diff`
